  ...
  <div class="pages">
    ...
    <Table :columns="columns" @on-selection-change="check" :data="tableData" :loading="loading"
      ...
      <template v-if="$route.params.status == '2'" slot="action" slot-scope="{row}" >
        <a   @click="change( row.id, row.shortName )">审核</a>
      </template>
    </Table>
    ...
    <singvideoDlg ref="addOrUpdatevideo" @done="dlgEditDone" />
  </div>
</template>
...
export default class Main extends Mixins(ViewBase, UrlManager) {
  ...
  get columns() {
    const data: any = [
      { title: '资源方公司名称', key: 'shortName',  align: 'center' },
      { title: '影院名称', key: 'hallCount', align: 'center' },
      { title: '广告片', key: 'seatCount', align: 'center' },
      { title: '投放周期',  key: 'code', align: 'center' },
      {
        title: '状态',
        key: 'status',
        align: 'center',
  ...
  changeAll(id: number, shortName: any) {
    this.videoVisible = true
    this.$nextTick(() => {
      (this.$refs.addOrUpdatevideo as any).init(id, shortName)
    })
  }
