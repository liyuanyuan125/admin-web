    ...
    <div class='page'>
        ...
        <Row class='bos'>
            <Row>
                <Col :span='2'>设置折扣</Col>
                <Col :span='4'>
                <Input style="width:100px" disabled v-model="dataplan.discount">
                </Input>
                </Col>
            </Row>
            <Row>
                <Col :span='2'>定金</Col>
                <Col :span='4'>
                <Input style="width:100px" disabled v-model="dataplan.depositAmount">
                </Input>
                </Col>
            </Row>
            <Row v-if='$route.params.ifs == 0'>
                <Col :span='2'>应结金额</Col>
                <Col :span='3'>
                <Input style="width:100px" :disabled='this.$route.params.ifs != 1 || (listitem.status != 9 && listitem.status != 10)' v-model="dataplan.needPayAmount">
                </Input>
                </Col>
                <Col :span='10' v-if='listitem.totalCost && listitem.totalCost != null'>实际投放花费 ￥ ({{formatNumber(listitem.totalCost)}})</Col>
            </Row>
            <Row v-else-if='$route.params.status != 6 && $route.params.status != 7 && $route.params.status != 3'>
                <Col :span='2'>应结金额</Col>
                <Col :span='3'>
                <Input style="width:100px" :disabled='this.$route.params.ifs != 1 || (listitem.status != 9 && listitem.status != 10)' v-model="dataplan.needPayAmount">
                </Input>
            ...
            <Row v-if='$route.params.ifs == 1 && $route.params.status != 6 && $route.params.status != 7 && $route.params.status != 3'>
                <Col :span='2'>确认应结金额</Col>
                <Col :span='3'>
                <Input style="width:100px" :disabled='this.$route.params.ifs != 1 || (listitem.status != 9 && listitem.status != 10)' v-model="dataplan.qneedPayAmount">
                </Input>
                </Col>
               <!--  <Col :span='10' v-if='listitem.totalCost && listitem.totalCost != null'>实际投放花费 ￥ ({{formatNumber(listitem.totalCost)}})</Col> -->
            </Row>
        ...
        <div style='padding: 20px 0 30px 0'>
            <Form ref="dataplan" :model="dataplan" :rules="ruleValidate" label-position="left" :label-width="100">
                ...
                <span v-if=' (listitem.totalAmount != null && this.$route.params.ifs == 0) && (listitem.discount != 1 && listitem.discount != "" && listitem.discount != null)'>, 折扣后总价【{{formatNumber(listitem.totalAmount , 2)}}】</span>
...
export default class Main extends ViewBase {
    ...
    async save(dataplan: any) {
        ...
        if (this.listitem.status == 9) { // 核对应结金额
    ...
    async chgposition() {
        const addlist = await deliveryposition(this.$route.params.id , {deliveryPositionCode: this.placement.position})
    }
